...
def get_numbers(array)
  ...
  array.each do |mini_arr|
    arr_unique = mini_arr.select { |arr| arr.length == 2 || arr.length == 4 || arr.length == 3 || arr.length == 7 }
    arr_nonunique = mini_arr - arr_unique
    hash_final = unique(arr_unique)
    hashes << hash_final.merge(non_unique(arr_nonunique, hash_final))
  end
  return hashes
end
...
def output_digits(output, hashes)
  ...
  output.each_with_index do |moutput, index|
...
to_sum = output_digits(output, hashes)

final = to_sum.map { |mini_sum| mini_sum.join("").to_i }
p final.sum
